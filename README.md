...
- `AP1_code` - The code of the previous semester server.
  - `files` - The files that are uploaded to the previoues semester server.
...
1. Run the anomaly detection server from the previous semester. In order to do it, you will need to compile the code in the AP1_code and run it. Follow the following steps:
   ...
   **WARNING:** Pay attention that our server runs on port 8080 so make sure you don't use this port (if you run both of the servers on the same machine of course).
